...
lemma pointwiseStabilizer_fixedPoints (β : Set α) :
  ...
  fun _ ha ⟨_, hm⟩ => (mem_pointwiseStabilizer_iff M β).mp hm _ ha
...
@[to_additive (attr := simp)]
lemma mem_setwiseStabilizer_iff (β : Set α) :
...
lemma pointwiseStabilizer_le_setwiseStabilizer (β : Set α) :
  ...
  fun _ hm => (mem_setwiseStabilizer_iff M β).mpr
    fun _ ha => Eq.subst (motive := (fun a => a ∈ β))
      (((mem_pointwiseStabilizer_iff M β).mp hm) _ ha).symm ha
...
lemma pointwiseStabilizer_le {β₁ β₂ : Set α} (h : β₁ ⊆ β₂):
  ...
  fun _ hm => (mem_pointwiseStabilizer_iff M β₁).mpr
    fun _ ha => ((mem_pointwiseStabilizer_iff M β₂).mp hm) _ <| h ha
